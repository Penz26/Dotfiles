...
local function default_config()
	return {
		communication = {
			whatsapp = {
				...
				match = {
					...
					{ initial_title = "WhatsApp Desktop" },
					{ title = "WhatsApp" },
				},
				command = { "whatsapp-linux-desktop" },
				move = true,
			},
		...
		music = {
			supersonic = {
				enable = true,
				match = { { class = "supersonic" }, { class = "supersonic-desktop" } },
				command = { "/usr/local/bin/supersonic" },
				move = true,
			},
		},
		sysmon = {
...
local function load_toggle_config()
	...
	if recognized and type(conf_or_error) == "table" then
		...
		hl.exec_cmd(
			"caelestia shell toaster error " .. shell_join({ "Failed to parse CLI config", reason }) .. " error"
		)
	end
			...
			elseif app.move then
				for _, target_client in ipairs(target_clients) do
					if not target_client.is_in_place then
						hl.dispatch(
							hl.dsp.window.move({ window = target_client.window, workspace = target, follow = false })
						)
					end
				end
			end
